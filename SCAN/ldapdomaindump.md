...
┌──(kali㉿kali)-[~/ldap_tmp]
└─$ ldapdomaindump -h
usage: ldapdomaindump [-h] [-u USERNAME] [-p PASSWORD] [-at {NTLM,SIMPLE}] [-o DIRECTORY] [--no-html] [--no-json] [--no-grep] [--grouped-json] [-d DELIMITER] [-r]
                      [-n DNS_SERVER] [-m]
                      HOSTNAME
...
```
## Enumlation
`ldapdomaindump ldap://10.10.11.35 -u 'cicada.htb\emily.oscars' -p 'Q!3@Lp#M6b*7t*Vt'`
